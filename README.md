...
学术辅助平台：多源输入（Assessment / 背景 / 大纲 / 具体要求）→ Summarizer 定稿 → **按章 IEEE/ACM 检索（可 LLM 造词）并确认入 Zotero** → LangGraph 按锁定大纲生成 APA 7th 草稿（**A/D 硬约束、大纲表结构保真、可选套用指定文档**；文献可选）；**精修工作区**支持节内多轮、跨节 Facts、确认时落库指令与重建 References；支持版本控制与 Word/PDF 导出导入。
...
5. **运行学术 Agent**（需 A + 锁定 C；文献可选；注入 directives / Facts；A 与 D 同为硬约束；大纲表保真；D 点名文档可升格注入）→ Draft  
...
5. **APA 草稿（LLM）** — A/D 同级硬约束 + 大纲结构/表保真 + 字数压缩；可选 must_apply 文档；引用护栏；directives / Facts 注入  
...
| [docs/20260727-writer-input-priority-discussion.md](./docs/20260727-writer-input-priority-discussion.md) | Writer 输入优先级 / 结构保真（W1–W4 已入 1.3.1） |

## 已知限制

- Writer / Z5 / 精修依赖 OpenAI 额度与网络；失败自动回退  
- 长度请用 **词数** 写在 D；页数仅为弱信号（不做硬校验）  
- must_apply 文档匹配失败、确认时未匹配引用：目前仅警告不阻断  
- 文档内嵌图 OCR / 真插图生成未做  
